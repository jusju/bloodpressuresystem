...
public class BloodPressureTrackerApplication {
	...
	public CommandLineRunner bloodPressureDemo(BloodPressureRepository repository, UserRepository urepository) {
		return (args) -> {
			...
			repository.save(new BloodPressure("1.1.2021", 120, 80, 72, user1));
			repository.save(new BloodPressure("2.1.2021", 135, 78, 81, user1));
			repository.save(new BloodPressure("3.1.2021", 118, 93, 75, user2));
			repository.save(new BloodPressure("4.1.2021", 140, 90, 63, user2));
			
			log.info("get blood pressure");
			for (BloodPressure pressure : repository.findAll()) {
